...
void WriMatrix(int **pM, int size,FILE* fp1);
...
int main()
{
    ...
    FILE* fp1 = NULL;
    errno_t err;
    const char* filename = "MatrixC.txt";
    const char* filename1 = "MatrixA.txt";
    if((err =fopen_s(&fp,filename, "w")) != 0) {
        printf("fail to open file");
        exit(0);
    }
    if((err = fopen_s(&fp1,filename1, "w"))!= 0) {
        printf("fail to open file");
        exit(0);
    }
    ...
    fprintf(fp1,"\nMatrix A:\n");
    WriMatrix(MatrixA,MatrixSize, fp1);
    fprintf(fp1,"\n");
    fprintf(fp1,"\nMatrix B:\n");
    WriMatrix(MatrixB, MatrixSize, fp1);
    ...
    cout<<"\nMatrix Result... \n";
    fprintf(fp, "Matrix C(result):\n");
    WriMatrix(MatrixC,MatrixSize, fp);

    cout<<"Matrix size "<<MatrixSize;
    cout<<"\nNormal mode "<<(endTime_For_Normal_Multipilication - startTime_For_Normal_Multipilication)<<" Clocks.."<<(double)(endTime_For_Normal_Multipilication - startTime_For_Normal_Multipilication)/CLOCKS_PER_SEC<<" Sec";
    cout<<"\nStrassen mode "<<(endTime_For_Strassen - startTime_For_Strassen)<<" Clocks.."<<(double)(endTime_For_Strassen - startTime_For_Strassen)/CLOCKS_PER_SEC<<" Sec\n";
    system("Pause");
    return 0;
...
void FillMatrix( int** MatrixA, int** MatrixB, int length)
{
    for(int row = 0; row<length; row++)
    {
        for(int column = 0; column<length; column++)
        {
            ...
            MatrixB[row][column] = (MatrixA[row][column] = rand() %10);
